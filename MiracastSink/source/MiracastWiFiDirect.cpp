...
MiracastWiFiDirect::MiracastWiFiDirect() : m_loop(nullptr), m_session(nullptr)
...
void MiracastWiFiDirect::Start(evpp::EventLoop* loop)
...

{
    ...
    std::wcout << "OnConnected" << std::endl;
    ...
    std::thread th([conn, name, this](wchar_t* mac) {
...

        {
            ...
            std::shared_ptr<MiracastSink> miracastSink = std::make_shared<MiracastSink>(m_loop);
...

{
    ...
    std::wcout << "OnDisconnected" << std::endl;
